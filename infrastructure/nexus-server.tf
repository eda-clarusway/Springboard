...
# By using the ssh key in the Jenkins server which is under .ssh folder, Jenkins server will connect to Nexus server as bastian host.
# Rancher IP address will come from aws_instance.tf-nexus-server.public_ip
# By using docker copy, it will access nexus container and it will copy the admin password and using cat it will read and using redirect it will write it to a file named initialpasswd.txt
